import \
    numpy as np  # Importiert das NumPy-Modul, das für numerische Operationen und die Arbeit mit Arrays verwendet wird.
import random
import \
    logging  # importiert das Modul logging, mit dem wir ein Verlaufsprotokoll über die Aktivitäten des Simulators erstellen können
import \
    argparse  # importiert das Modul argparse, wird benötigt um die Kommandozeilenargumente der Main zu analysieren und interpretieren
...
def input_Ressourcenvektor():
    ...
    anzahl = IntPrompt.ask(
        "Wie viele Ressourcenklassen haben sie ")  # Fragt den Benutzer nach der Anzahl der Ressourcenklassen.
    ...
    for i in range(anzahl):  # Iteriert über die Anzahl der Ressourcenklassen.
        ressource = IntPrompt.ask(
            f"Geben sie nun die Menge der Ressourcenklasse {i + 1} ")  # Fragt nach der Menge der aktuellen Ressourcenklasse.
...
def input_Belegungsmatrix(anzahl):
    ...
    belegungsmatrix = np.zeros((prozesse, anzahl),
                               dtype=int)  # Erstellt eine Matrix aus Nullen mit der Größe (Prozesse, anzahl).
    ...
    for i in range(prozesse):  # Iteriert über die Anzahl der Prozesse.
        ...
        for j in range(anzahl):  # Iteriert über die Anzahl der Ressourcenklassen.
            vektor = IntPrompt.ask(
                f"Geben sie an wie viele Ressourcen der Prozess {prozessanzahl} von der Ressourcenklasse {j + 1}")  # Fragt nach der benötigten Menge der aktuellen Ressourcenklasse für den aktuellen Prozess.
...
def input_Anforderungsmatrix(Prozesse, anzahl):
    ...
    for i in range(Prozesse):
        ...
        for l in range(anzahl):
            ...
            vektor = IntPrompt.ask(
                f"Geben sie an wie viele Ressourcen der Prozess {prozessanzahl} von der Ressourcenklasse {l + 1} benötigt")
            ...
            numpy_Anforderung = np.array(
                anforderung)  # Konvertiert die Liste in ein NumPy-Array (wird hier aber nicht verwendet)
...
def simulate_processes(ressourcenvektor, belegungsmatrix, anforderungsmatrix, noninteractive, logger):
    ...
    while not all(finished):
        ...
        if noninteractive:
            ...
            logger.info(
                f"Benutzer hat \"noninteractive\" gewählt, zufällige Wahl eines ausführbaren Prozesses getroffen: {nächster_prozess}")
        ...
        # nicht ausgewählt wird. Der Code muss erweitert werden, um die Ausführung fortzusetzen.
            ...
            while True:  # Beginne eine unendliche Schleife, die solange läuft, bis sie manuell unterbrochen wird
                console.print(
                    f"[blue]Ausführbare Prozesse: {ausführbare_prozesse}[/blue]")  # Drucke die Liste der ausführbaren Prozesse
                try:  # Versuche den folgenden Block auszuführen, um potenzielle Fehler abzufangen
                    nächster_prozess = IntPrompt.ask(
                    ...
                    console.print(
                        "[red]Ungültige Eingabe. Bitte eine gültige Prozessnummer eingeben.[/red]")  # Informiere den Benutzer über die ungültige Eingabe
        ...
        if any(neue_ressourcenrestvektor < 0):  # Wenn irgendein Wert im neuen Vektor negativ ist
            console.print(
                f"[red]Fehler: Negative Werte im Ressourcenrestvektor nach Ausführung von Prozess {nächster_prozess}.[/red]")  # Fehlerausgabe
            logger.error(
                f"Fehler: Negative Werte im Ressourcenrestvektor nach Ausführung von Prozess {nächster_prozess}")
            console.print(
                f"[yellow]Aktueller Ressourcenrestvektor: {ressourcenrestvektor}[/yellow]")  # Drucke den aktuellen Ressourcenrestvektor
            ...
            console.print(
                f"Belegte Ressourcen von Prozess {nächster_prozess}: {belegungsmatrix[nächster_prozess]}")  # Drucke die belegten Ressourcen des Prozesses
            logger.error(f"Belegte Ressourcen von Prozess {nächster_prozess}: {belegungsmatrix[nächster_prozess]}")
            break  # Beende die Schleife

        console.print(
            f"[yellow1]Ressourcenrestvektor nach Ausführung von Prozess {nächster_prozess}: {ressourcenrestvektor}[/yellow1]")  # Drucke den Ressourcenrestvektor nach der Ausführung des Prozesses
        ...
        console.print(
            f"[bright_magenta]Prozess {nächster_prozess} ist abgeschlossen. Ressourcenrestvektor: {ressourcenrestvektor}[/bright_magenta]")  # Informiere über den Abschluss des Prozesses und drucke den aktuellen Ressourcenrestvektor
        logger.info(f"Prozess {nächster_prozess} abgeschlossen, Ressourcenrestvektor: {ressourcenrestvektor}")

        console.print(
            "[chartreuse3]Simulation abgeschlossen. Alle ausführbaren Prozesse wurden bearbeitet [/chartreuse3]")  # Drucke eine Abschlussmeldung der Simulation
